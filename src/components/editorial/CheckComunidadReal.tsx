  ...
  const verdict =
    allAnswered
      ? noCount >= 3
        ...
        : { label: "Comunidad real", color: "#c8ff00", description: "Menos de 3 negativas. Hay conexión horizontal genuina." }
      : null;
        ...
        {QUESTIONS.map((q, i) => {
          ...
          return (
            <div
              ...
              style={
                ans === "si"
                  ? { borderColor: "rgba(200,255,0,0.25)", background: "rgba(200,255,0,0.03)" }
                  : ans === "no"
              ...
              <div className="flex gap-2" role="group" aria-label={`Respuesta a: ${q}`}>
                {(["si", "no"] as const).map((val) => (
                  <button
                    ...
                    style={{
                      border: `1px solid ${ans === val ? (val === "si" ? "#c8ff00" : "#ff6b6b") : "#333"}`,
                      background: ans === val ? (val === "si" ? "rgba(200,255,0,0.12)" : "rgba(255,107,107,0.12)") : "transparent",
                      color: ans === val ? (val === "si" ? "#c8ff00" : "#ff6b6b") : "#555",
                    }}
                  >
                    {val === "si" ? "Sí" : "No"}
                  </button>
